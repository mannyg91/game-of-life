...
def renderCells(matrix, muted, newCells):
    ...
    if sequencer == False and muted == False:
        for row, col in zip(*newIndices):
            notesCell(row, col, cellSize, volume)
...
def playColumn(matrix, col, noteDict, volume):
    playCol = matrix[:, col-1]
    return notes(playCol, noteDict, col-1, volume)
...
def notes(playCol, noteDict, col, volume):
    notes = []
...
def noteList(cols):
    ...
    notes = [127, 124, 122, 120, 117, 115, 112, 110, 108, 105, 103, 100, 98, 96, 93, 91, 88, 86, 84, 81, 79, 76, 74, 72, 69, 67, 64, 62, 60, 57, 55, 52, 50, 48, 45, 43, 40, 38, 36, 33, 31, 28, 26, 24, 21]
...
def notesCell(row, col, cellSize, volume, noteDict = noteDict):
    note = noteDict[col]
    # screen.blit(musicSurf2,(col*cellSize,row*cellSize))
...
while 1:
    ...
    if sequencer == True and paused == False:
        playColumn(matrix, colNum, noteDict, volume)
        colNum += 1
        if colNum == cols:
            colNum = 0
